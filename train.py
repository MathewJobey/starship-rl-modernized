...
if __name__ == '__main__':
    ...
    task = 'landing'  # We changed this to 'landing' for the bellyflop!
    ...
    if len(glob.glob(os.path.join(ckpt_folder, '*.pt'))) > 0:
        # load the last ckpt
        checkpoint = torch.load(glob.glob(os.path.join(ckpt_folder, '*.pt'))[-1],weights_only=False)  # load the most recent checkpoint
